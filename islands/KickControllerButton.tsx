...
export default function KickControllerButton({ user, clientId, activeControllerClientId }: KickControllerButtonProps) {
  ...
  const kickResult = useSignal<{ success: boolean; message: string } | null>(null);
  ...
  function handleButtonPress() {
    ...
    kickTimer.value = window.setInterval(() => {
      ...
      if (progress >= 100) {
        window.clearInterval(kickTimer.value!);
  ...
  async function executeKick() {
    try {
      ...
      setTimeout(() => {
        ...
        window.location.href = `/ctrl?active=true&clientId=${encodeURIComponent(clientId)}&t=${Date.now()}`;
      }, 1000);
  ...
  return (
    <div class="kick-controller-container">
      {kickResult.value ? (
        <div class={`kick-result ${kickResult.value.success ? 'kick-success' : 'kick-error'}`}>
