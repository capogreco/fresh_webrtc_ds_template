...
import { AudioEngineService, type SynthParams } from "../../services/AudioEngineService.ts";
...
export function useAudioEngine(
  ...
  const initializeAudioContext = useCallback(async () => {
    try {
      ...
      addLog(`Error initializing audio context: ${error instanceof Error ? error.message : String(error)}`);
  ...
  const startPinkNoise = useCallback((gain: number = 0.2) => {
    try {
      if (audioEngineRef.current && audioReady.value) {
        audioEngineRef.current.startPinkNoise(gain);
        pinkNoiseActive.value = true;
        addLog("Pink noise started for volume check");
      }
    } catch (error) {
      addLog(`Error starting pink noise: ${error instanceof Error ? error.message : String(error)}`);
  ...
  const updateSynthParam = useCallback((param: keyof SynthParams, value: any) => {
