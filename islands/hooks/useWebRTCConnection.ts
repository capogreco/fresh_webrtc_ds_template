...
export function useWebRTCConnection(
  ...
  const connectionStatus = computed(() => {
    ...
    if (isConnected.value && dataChannel.value?.readyState === "open") {
      return "connected";
    }
    if (
      connectionState.value === "connecting" || connectionState.value === "new"
    ) return "connecting";
    if (
      connectionState.value === "failed" || connectionState.value === "closed"
    ) return "disconnected";
    return "initializing";
  });
    ...
    channel.onmessage = (event) => {
      try {
        ...
        if (typeof parsedData === "object") {
          ...
          addLog(
            `Received: ${String(event.data).substring(0, 50)}${
              String(event.data).length > 50 ? "..." : ""
            }`,
          );
        }
      } catch (err) {
