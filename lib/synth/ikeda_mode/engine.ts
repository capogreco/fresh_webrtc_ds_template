...
export class IkedaModeMVPEngine {
  ...
  private paramHandlers: Map<string, (value: unknown) => void>;
  ...
  constructor(
    ...
    // Temporarily using console.log to bypass this.logger for debugging
    console.log("[IkedaMVPEngine CONSTRUCTOR_LOG] CONSTRUCTOR CALLED: IkedaModeMVPEngine (Simplified MVP)");
    console.log("CONSTRUCTOR CALLED: IkedaModeMVPEngine (Simplified MVP)"); // Keep original console.log for comparison

    this.params = { ...IKEDA_MVP_ENGINE_DEFAULTS, ...(initialParams || {}) };
    // Restore this.logger for this message
    ...
    this.paramHandlers.set(
      "ikedaGlobalOnOff",
      this.setIkedaGlobalOnOff.bind(this),
    );
    this.paramHandlers.set(
      "ikedaPinkNoiseLevel",
      this.setIkedaPinkNoiseLevel.bind(this),
    );
    this.paramHandlers.set(
      "ikedaVolumeCheckLevel",
      this.setIkedaVolumeCheckLevel.bind(this),
    );
  ...
  private async _generatePinkNoiseBufferAndSetup() {
    ...
    this.logger(
      `// DEBUG-AUDIO: AudioContext state is ${this.audioContext.state}`,
    );
    ...
    this.logger(
      `// DEBUG-AUDIO: Creating pink noise buffer with ${frameCount} frames (${duration}s) at ${sampleRate}Hz`,
    );
    ...
    // Log connection path
    this.logger(
      "// DEBUG-AUDIO: Connecting pinkNoiseSource -> pinkNoiseGain -> engineOutputGain",
    );
    ...
    try {
      if (this.audioContext.state === "running") {
        ...
        this.logger(
          `// DEBUG-AUDIO: PinkNoiseSource started. AudioContext state: ${this.audioContext.state}. PinkNoiseGain value: ${this.pinkNoiseGain.gain.value}`,
        );
      } else {
        this.logger(
          `// DEBUG-AUDIO: AudioContext not running (state=${this.audioContext.state}), PinkNoiseSource not started yet.`,
        );

        // Attempt to start if context resumes later
        const startOnResume = () => {
          this.logger(
            `// DEBUG-AUDIO: AudioContext state changed to: ${this.audioContext.state}`,
          );
          if (
            this.audioContext.state === "running" && this.pinkNoiseSource &&
            !this.pinkNoiseSource.context
          ) {
            try {
              this.pinkNoiseSource.start();
              this.logger(
                `// DEBUG-AUDIO: PinkNoiseSource started on context resume. PinkNoiseGain value: ${this.pinkNoiseGain.gain.value}`,
              );
            } catch (e) {
              this.logger(
        ...
        this.logger(
          "// DEBUG-AUDIO: Set up listener for AudioContext state changes",
        );
      }
    } catch (e) {
      this.logger(
    ...
    this.logger(
      `// DEBUG-AUDIO: Calling _applyActiveStateAudio to set initial gain`,
    );
    this._applyActiveStateAudio();
    this.logger("// DEBUG-AUDIO: _generatePinkNoiseBufferAndSetup EXITED");
  }

  public updateParam(paramId: string, value: unknown): void {
    this.logger(`updateParam: id='${paramId}', value='${String(value)}'`);
    const handler = this.paramHandlers.get(paramId);
    if (handler) {
      handler(value);
    } else {
      this.logger(
  ...
  private setIkedaGlobalOnOff(value: unknown): void {
    const booleanValue = Boolean(value);
    if (this.params.ikedaGlobalOnOff !== booleanValue) {
      this.params.ikedaGlobalOnOff = booleanValue;
      this.logger(`Param set: ikedaGlobalOnOff = ${this.params.ikedaGlobalOnOff}`);
      this._applyActiveStateAudio();
    }
  }

  private setIkedaPinkNoiseLevel(value: unknown): void {
    const numericValue = Number(value);
    if (isNaN(numericValue)) {
      this.logger(`Invalid numeric value for ikedaPinkNoiseLevel: ${value}`);
      return;
    }
    const clampedValue = Math.max(0, Math.min(1, numericValue)); // Clamp
    if (this.params.ikedaPinkNoiseLevel !== clampedValue) {
      this.params.ikedaPinkNoiseLevel = clampedValue;
      this.logger(`Param set: ikedaPinkNoiseLevel = ${this.params.ikedaPinkNoiseLevel}`);
      this._applyActiveStateAudio();
    }
  }

  private setIkedaVolumeCheckLevel(value: unknown): void {
    const numericValue = Number(value);
    if (isNaN(numericValue)) {
      this.logger(`Invalid numeric value for ikedaVolumeCheckLevel: ${value}`);
      return;
    }
    // Clamp according to param definition in shared/modes/ikeda/params.ts (min: 0.01, max: 0.5)
    const clampedValue = Math.max(0.01, Math.min(0.5, numericValue));
    if (this.params.ikedaVolumeCheckLevel !== clampedValue) {
      this.params.ikedaVolumeCheckLevel = clampedValue;
      this.logger(`Param set: ikedaVolumeCheckLevel = ${this.params.ikedaVolumeCheckLevel}`);
      // Only apply audio state if volume check is currently pending,
      // otherwise this parameter doesn't have an immediate effect on sound.
  ...
  private _applyActiveStateAudio(): void {
    this.logger(
      `// DEBUG-AUDIO: _applyActiveStateAudio: isVolumeCheckPending=${this.isVolumeCheckPending}, params.ikedaGlobalOnOff=${this.params.ikedaGlobalOnOff}`,
    );

    if (!this.pinkNoiseGain || !this.audioContext) {
      this.logger(
        "// DEBUG-AUDIO: _applyActiveStateAudio: Missing pinkNoiseGain or audioContext, cannot continue",
      );
    ...
    if (this.isVolumeCheckPending) {
      ...
      this.logger(
        `// DEBUG-AUDIO: _applyActiveStateAudio: Volume check active. Setting pinkNoiseGain.gain to ${targetGainValue}`,
      );
      ...
      if (!this.isGenerativeAudioActive) {
        this.logger(
          "// DEBUG-AUDIO: _applyActiveStateAudio: Starting generative audio (MVP: pink noise)",
        );
      }
      this.logger(
        `// DEBUG-AUDIO: _applyActiveStateAudio: Setting pinkNoiseGain.gain to ${targetGainValue}`,
      );
      this.pinkNoiseGain.gain.setTargetAtTime(
        targetGainValue,
        currentTime,
      ...
      if (this.isGenerativeAudioActive) {
        this.logger(
          "// DEBUG-AUDIO: _applyActiveStateAudio: Stopping generative audio",
        );
      }
      this.logger(
        `// DEBUG-AUDIO: _applyActiveStateAudio: Setting pinkNoiseGain.gain to ${targetGainValue}`,
    ...
    this.logger(
      `// DEBUG-AUDIO: _applyActiveStateAudio: Current pinkNoiseGain.gain.value=${currentGainValue}, target=${targetGainValue}`,
    );
  }
  ...
  public getOutputNode(): AudioNode {
    this.logger(
      `// DEBUG-AUDIO: getOutputNode called, returning engineOutputGain`,
    );
    if (this.engineOutputGain) {
      // Log the connected nodes if possible
      try {
        this.logger(
          `// DEBUG-AUDIO: engineOutputGain exists and will be returned`,
        );
      } catch (error) {
        this.logger(
          `// DEBUG-AUDIO: Error checking engineOutputGain connections: ${error}`,
        );
      }
    } else {
      this.logger(
        `// DEBUG-AUDIO: WARNING - engineOutputGain is null or undefined!`,
      );
    }
